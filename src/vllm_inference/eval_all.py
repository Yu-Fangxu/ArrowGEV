...
def find_dataset_results(dataset_name, split, model_name):
    ...
    """
    data_dirs = []
    eval_root = f"/mnt/gemininjceph3/geminicephfs/pr-others-prctrans/fangxuyu/time-r1/logs/eval/{model_name}"
    for data_dir in os.listdir(eval_root):
        if dataset_name in data_dir:
            # data_dirs.append(os.path.join(eval_root, data_dir)+"_reverse")
...
def compute_IoU(pred, gt, duration):
    ...
    # gt = np.array([duration, duration]) - gt
        ...
        for line in open(file_path):
            ...
            if datasetname in ["activitynet", "charades", "tvgbench"]:
                ...
                if None not in tmp["pred"]:
                    score = compute_IoU(tmp["pred"], tmp["target"], tmp["duration"])
            else:
