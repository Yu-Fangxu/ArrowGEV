...
for FILTER_INDEX in {0..0}; do
    ...
    if [ $FILTER_INDEX -eq 0 ]; then
        ...
        DATA_PATH=/mnt/gemininjceph3/geminicephfs/pr-others-prctrans/fangxuyu/time-r1/logs/$EXP_NAME/$PREV_WANDB_NAME/filtering_epoch$((FILTER_INDEX-1))/train_v4_cloud_0071_all.json
        LOAD_MODEL_PATH="/mnt/gemininjceph3/geminicephfs/pr-others-prctrans/fangxuyu/time-r1/logs/$EXP_NAME/${PREV_WANDB_NAME}/train_epoch$((FILTER_INDEX-1))/$min_checkpoint_name"
    fi
    ...
    # 设置路径变量
    OUTDIR=/mnt/gemininjceph3/geminicephfs/pr-others-prctrans/fangxuyu/time-r1/logs/$EXP_NAME/${WANDB_NAME}/train_epoch$FILTER_INDEX
    export LOG_PATH="/mnt/gemininjceph3/geminicephfs/pr-others-prctrans/fangxuyu/time-r1/${OUTDIR}/log.txt"
    mkdir -p $OUTDIR
    ...
    CUDA_VISIBLE_DEVICES=$GPU_LIST torchrun --nproc_per_node="8" \
        ...
        --master_port="12371" \
        /mnt/gemininjceph3/geminicephfs/pr-others-prctrans/fangxuyu/time-r1/main.py \
        --deepspeed /mnt/gemininjceph3/geminicephfs/pr-others-prctrans/fangxuyu/time-r1/scripts/zero3_offload.json \
        --output_dir $OUTDIR \
    ...
    # 推理阶段
    # vllm_output_dir=/mnt/gemininjceph3/geminicephfs/pr-others-prctrans/fangxuyu/time-r1/logs/$EXP_NAME/$WANDB_NAME/filtering_epoch$FILTER_INDEX
    # mkdir -p "$vllm_output_dir"

    # # # 查找最小checkpoint
    # checkpoints_parent_path="/mnt/gemininjceph3/geminicephfs/pr-others-prctrans/fangxuyu/time-r1/logs/${EXP_NAME}/${WANDB_NAME}/train_epoch$FILTER_INDEX"
    ...
    # wait
    # 数据处理
    # python /mnt/gemininjceph3/geminicephfs/pr-others-prctrans/fangxuyu/time-r1/src/vllm_inference/calc_difficulty.py --input $vllm_output_dir \
    #     --split $DATA_PATH \
    #     --output_dir "/mnt/gemininjceph3/geminicephfs/pr-others-prctrans/fangxuyu/time-r1/"
    # # 生成过滤后的数据
    # python /mnt/gemininjceph3/geminicephfs/pr-others-prctrans/fangxuyu/time-r1/src/utils/process_data.py --input_json $vllm_output_dir/train_v4_cloud.json --task 0071_all
